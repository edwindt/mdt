  ...
  <div>
    ...
    <v-dialog v-model="showImportDialog" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <data-import @imported="handleImportedData" />
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
import DataImport from './DataImport.vue';  // Import the DataImport component

export default {
  components: {
    DataImport,  // Register the DataImport component
  },
  setup() {
    ...
    const showImportDialog = ref(false);  // State to control the import dialog
    const fileName = ref('');
